...
#include "../algoritms/dictonary.h"


template <typename K, typename V>
void AssertSingleValueInsertion(Dictonary<K, V>& dict, K key, V value)
{
    dict.put(key, value);
...
template <typename K, typename V>
void AssertSingleValueRemoval(Dictonary<K, V>& dict, K key)
{
    dict.remove(key);
...
TEST(DictTest, TestSingleIsnertion) {
    ...
    AssertSingleValueInsertion(dict, (std::string)"wow", (std::string)"is it working?");
...
TEST(DictTest, TestIsnertion) {
    ...
    for (int i = 0; i < 20; i++)
    {
        ...
        AssertSingleValueInsertion(dict, key, i);
    }
...
TEST(DictTest, TestRemove) {
    ...
    AssertSingleValueInsertion(dict, key, 22);
    EXPECT_EQ(dict.size(), 1);
    AssertSingleValueRemoval(dict, key);
